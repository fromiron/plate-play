...
import type { LocalizedString, Theme, Promotion } from "@/lib/types";
...
const ThemeSchema = z.object({
  ...
  fontPair: z.enum(["inter-playfair", "inter-merriweather", "inter-roboto-slab"]).optional(),
...
export const menuBoardRouter = createTRPCRouter({
      ...
      const board = await ctx.db.$transaction(async (tx) => {
        ...
        const newBoard = await tx.menuBoard.create({
          data: {
            ...
            description: input.description ? serializeJSON(input.description) : undefined,
            currency: input.currency,
            defaultLang: input.defaultLang,
            theme: input.theme ? serializeJSON(input.theme) : undefined,
            promotions: input.promotions?.length ? serializeJSON(input.promotions) : undefined,
            createdById: ctx.session.user.id,
          }
        });
  ...
  getStats: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const totalItems = board.sections.reduce((sum, section) => sum + section.items.length, 0);
      const soldOutItems = board.sections.reduce((sum, section) => 
        sum + section.items.filter(item => item.status === 'soldout').length, 0
      );
