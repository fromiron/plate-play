...
export function suggestPalette(base: string) {
  try {
    ...
    const primary = base
    const secondary = hslToHex((h + 200) % 360, Math.min(90, s + 10), Math.min(90, l + 10))
    const accent = hslToHex((h + 40) % 360, Math.min(95, s + 20), Math.max(30, l - 10))
    return { primary, secondary, accent }
  } catch {
    return { primary: base || "#16a34a", secondary: "#0ea5e9", accent: "#f59e0b" }
  }
}
