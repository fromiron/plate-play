...
export default function BoardEditorPage() {
	...
	const {
		...
		refetch,
		error,
	} = api.menuBoard.getById.useQuery({ id: params.id });
			...
			<div className="mb-4 flex flex-col items-start justify-between gap-3 md:mb-6 md:flex-row md:items-center">
				...
				<div className="flex flex-wrap items-center gap-2">
					<div className="flex items-center gap-2">
						...
						/>
						<Label className="ml-2 text-muted-foreground text-xs">
							출력 언어
						</Label>
					...
					<Button
						variant="outline"
						onClick={() => handleSave(currentBoard)}
					...
					<Button
						onClick={() => setShareUrl(buildShareUrl(currentBoard))}
						className="inline-flex items-center gap-2"
					>
				...
				<Card id="qr" className="h-fit">
					...
					<CardContent className="grid gap-4">
						<Tabs defaultValue="local">
							...
							<TabsContent value="local" className="space-y-3">
								<Label className="text-muted-foreground text-xs">
									이 장치(LocalStorage)에서만 데이터가 보입니다.
								</Label>
								<div
									ref={qrRef}
									className="mx-auto w-full max-w-[220px] rounded-md bg-white p-3"
								>
						...
						<p className="text-muted-foreground text-xs">
							팁: 출력물에 QR을 배치해 손님이 휴대폰으로 메뉴를 볼 수 있게
							하세요.
						</p>
					</CardContent>
				</Card>
